...
class FiltersMod(loader.Module):
    """When you filter a text, it auto replys to it if a user triggers the word)"""
    strings = {
        "name": "Filters"
    }
    ...
    async def watchout(self, message):
        ...
        if chatid not in str(filters):
            return
        for key in [говно.lower() for говно in filters[chatid]]:
            if key.lower() in args.lower():
                ...
                if not value.media and not value.web_preview:
                    ...
                    if value.text.startswith(".") is False:
                        arg = value.text
                        exec = False
                    reply = await message.reply(arg)
                    if exec is True:
                        argspr = arg.split(" ")
                        reply.message, cmd = self.allmodules.dispatch(argspr[0], reply)
                        await cmd(reply)
                else:
